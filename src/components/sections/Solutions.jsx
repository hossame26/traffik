...
function MobileCard({ offer, onClick }) {
  return (
    <div
      onClick={onClick}
      className="flex flex-col p-5 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.02] w-[85vw] max-w-[320px] shadow-sm dark:shadow-none cursor-pointer"
    >
      {offer.badge && (
        <div className="self-start bg-gradient-to-r from-violet-500 to-purple-600 text-white text-[9px] font-bold tracking-wider px-3 py-1 rounded-full mb-4">
          {offer.badge}
        </div>
      )}

      <div className="w-16 h-16 rounded-xl bg-gray-100 dark:bg-white/5 p-3 mb-5">
        <img src={offer.image} alt={offer.title} className="w-full h-full object-contain" />
      </div>
      ...
      <button
        onClick={onClick}
        className={`w-full py-3.5 rounded-full bg-gradient-to-r ${offer.color} text-white text-sm font-semibold flex items-center justify-center gap-2`}
      >
        En savoir plus
        <ArrowRight className="w-4 h-4" />
      </button>
          ...
          >
            {offers.map((offer) => (
              <div key={offer.id} className="snap-center shrink-0">
                <MobileCard offer={offer} onClick={() => setSelectedOffer(offer)} />
              </div>
            ))}
          </div>
            ...
            {offers.map((offer, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-colors ${
                  index === currentSlide ? 'bg-primary dark:bg-white' : 'bg-gray-300 dark:bg-white/20'
                }`}
              />
            ))}
          </div>
